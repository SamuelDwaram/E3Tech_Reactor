...
begin
	...
	declare @temp nvarchar(10) = IIF(@Temperature is null, 0, @Temperature);

	declare @query nvarchar(max);
	set @query = 'insert into dbo.' + @FieldDeviceIdentifier + ' values(' + @i_R + ',' + @i_Y + ',' + @i_B + ',' + @v_1 + ',' + @v_2 + ',' + @v_3 + ',' + @vib + ',' + @temp + ',GetDate())';
...
begin
	...
	if @Temperature is not null
		set @query = @query + 'Temperature as [Temp(°C)],';
