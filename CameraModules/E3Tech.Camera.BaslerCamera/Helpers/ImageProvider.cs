...
namespace PylonC.NETSupportLibrary
{
    ...
    public class ImageProvider
    {
        ...
        /* Set the Acquisition Frame Rate of Camera */
        public void SetAcquistionRateOfCameraAndImageROI(double frameRate)
        {
            if (Pylon.DeviceFeatureIsAvailable(m_hDevice, "AcquisitionFrameRateEnable"))
            {
                // Set the width to the maximum value
                long maxWidth = Pylon.DeviceGetIntegerFeatureMax(m_hDevice, "Width");
                //Set the Width to 1440
                Pylon.DeviceSetIntegerFeature(m_hDevice, "Width", 1440);
                // Set the height to 900
                Pylon.DeviceSetIntegerFeature(m_hDevice, "Height", 900);

                Pylon.DeviceSetBooleanFeature(m_hDevice, "AcquisitionFrameRateEnable", true);
                Pylon.DeviceSetFloatFeature(m_hDevice, "AcquisitionFrameRateAbs", frameRate);
            }
        }
        ...
        public bool IsGrabbingVideo()
        {
            return m_open && m_grabThread.IsAlive && !m_grabOnce && m_grabThreadRun;
        }
        ...
        protected void Grab()
        {
            ...
            try
            {
                ...
                while (m_grabThreadRun) /* Is set to false when stopping to end the grab thread. */
                {
                    ...
                    if (!Pylon.StreamGrabberRetrieveResult(m_hGrabber, out PylonGrabResult_t grabResult))
                    {
        ...
        protected void EnqueueTakenImage(PylonGrabResult_t grabResult)
        {
            ...
            if (!m_buffers.TryGetValue(grabResult.hBuffer, out PylonBuffer<byte> buffer))
            {
            ...
            {
                Handle = grabResult.hBuffer /* Add the handle to requeue the buffer in the stream grabber queue. */
            };
            ...
            else /* Conversion is required. */
            {
                ...
                bool bufferListed = m_convertedBuffers.TryGetValue(grabResult.hBuffer, out PylonBuffer<byte> convertedBuffer);
        ...
        protected void CleanUpGrab()
        {
            ...
            /* ... The m_buffers can now be retrieved from the stream grabber. */
            {
                ...
                {
                    /* Stores the result of a grab operation. */
                    isReady = Pylon.StreamGrabberRetrieveResult(m_hGrabber, out PylonGrabResult_t grabResult);
        ...
        public delegate void DeviceOpenedEventHandler(object sender, EventArgs e);
        public event DeviceOpenedEventHandler DeviceOpenedEvent;

        public delegate void DeviceClosingEventHandler(object sender, EventArgs e);
        public event DeviceClosingEventHandler DeviceClosingEvent;

        public delegate void DeviceClosedEventHandler(object sender, EventArgs e);
        public event DeviceClosedEventHandler DeviceClosedEvent;

        public delegate void GrabbingStartedEventHandler(object sender, EventArgs e);
        public event GrabbingStartedEventHandler GrabbingStartedEvent;

        public delegate void ImageReadyEventHandler(object sender, EventArgs e);
        public event ImageReadyEventHandler ImageReadyEvent;

        public delegate void GrabbingStoppedEventHandler(object sender, EventArgs e);
        ...
        {
            m_open = true;
            DeviceOpenedEvent?.Invoke(this, new EventArgs());
        }
        ...
        {
            m_open = false;
            DeviceClosingEvent?.Invoke(this, new EventArgs());
        }
        ...
        {
            m_open = false;
            DeviceClosedEvent?.Invoke(this, new EventArgs());
        }
        ...
        {
            GrabbingStartedEvent?.Invoke(this, new EventArgs());
        }
        ...
        {
            ImageReadyEvent?.Invoke(this, new EventArgs());
        }

        /* Notify that grabbing has stopped. This event could be used to update the state of the GUI. */
        protected void OnGrabbingStoppedEvent()
        {
            GrabbingStoppedEvent?.Invoke(this, new EventArgs());
        }

        /* Notify that the grabbing had errors and deliver the information. */
        protected void OnGrabErrorEvent(Exception grabException, string additionalErrorMessage)
        {
            GrabErrorEvent?.Invoke(this, new GrabErrorEventArgs()
            {
                GrabException = grabException,
        ...
        protected void OnDeviceRemovedEvent()
        {
            m_removed = true;
            m_grabThreadRun = false;
            DeviceRemovedEvent?.Invoke(this, new EventArgs());
        }
    }
